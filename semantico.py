...
class Semantico:
    def __init__(self, nomeAlvo):
        # putchar, getchar, 
        self.escopos = [{}]  # Pilha de escopos, cada escopo é um dicionário
        # self.Alvo = open(nomeAlvo, "wt")
        self.declara("putint", (TOKEN.FUNCTION, [[TOKEN.INT, False], [TOKEN.INT, False]])) # putint(int x)
        self.declara("getint", (TOKEN.FUNCTION, [[TOKEN.INT, False]])) # getint()
        self.declara("putfloat", (TOKEN.FUNCTION, [[TOKEN.FLOAT, False],[TOKEN.FLOAT, False]])) # putfloat(float x)
        self.declara("getfloat", (TOKEN.FUNCTION, [[TOKEN.FLOAT, False]])) # getfloat()
        self.declara("putchar", (TOKEN.FUNCTION, [[TOKEN.CHAR, False], [TOKEN.CHAR, False]])) # putchar(char x)
        self.declara("getchar", (TOKEN.FUNCTION, [[TOKEN.CHAR, False]])) #getchar()
    ...
    def gera(self, nivel, codigo): # isso aqui não vai existir pois é coisa de identação do python né?
        identacao = ' ' * 4 * nivel
